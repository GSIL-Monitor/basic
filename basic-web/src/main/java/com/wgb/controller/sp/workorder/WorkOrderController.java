...
public class WorkOrderController extends SPBaseController {
    ...
    @ResponseBody
    public ZLResult finishService(){
        ZLResult result = ZLResult.Success();
        Map<String, Object> params = getParams();
        try {
            params.put("workorderstatus" ,"9");
            ZLRpcResult rpcResult = apitWorkOrderService.finishService(params);
            result = parseRpcResultForMsg(rpcResult);
        }catch (Exception ex){
            LOGGER.error("完成维修工单操作失败!",ex);
            result.setErrorMsg("操作失败，请联系管理员！");
        }
        return result;
    }
    ...
    @ResponseBody
    public ZLResult findWorkOrderPrice(){
        ZLResult result = ZLResult.Success();
        Map<String, Object> params = getParams();
        try {
