...
import com.wgb.service.dubbo.pays.admin.ApiLoadingDataService;
...
public class MTLoadingDataFomPAYSController extends MTBaseController {

    @Autowired
    private ApiLoadingDataService apiLoadingDataServiceFromPYAS;

    @RequestMapping("/getLoadingPaytypesData")
    @ResponseBody
    public ZLResult getLoadingPaytypesIdsData(HttpServletRequest request) {
        // 获取入参数据
        ...
        try {
            // 系统调用
            zlRpcResult = apiLoadingDataServiceFromPYAS.getLoadingPaytypesData(params);
        } catch (Exception ex){
            // 系统级别异常
