...
/**
 * Created by qiuxh on 2018/9/11.
 */
@Controller
@RequestMapping("/goods")
public class MTGoodsHelpLoginController extends MTBaseController{
    ...
    public ZLResult login(HttpServletRequest request) {
        ...
        if ((StringUtils.isNotEmpty(username) && StringUtils.isNotEmpty(shopcode)) ||
            ...
            //查询商户助手用户信息
            rpcResult = apiUserService.getLoginUserForGoodsHelp(params);
            ...
            if (resultMap != null) {
                ...
                //将account放到缓存中
                RedisFactory.getPassportClient().set(HttpRequestConstant.MT_GOODS_HELP_ACCOUNT_PREFIX + token, MapUtils.getString(userInfo, "account"), SESSION_TIME_SECONDS);
    ...
    public ZLResult logout(HttpServletRequest request) {
        ...
        if (StringUtils.isNotEmpty(token)) {
            RedisFactory.getPassportClient().del(HttpRequestConstant.MT_GOODS_HELP_ACCOUNT_PREFIX + token);
        }
