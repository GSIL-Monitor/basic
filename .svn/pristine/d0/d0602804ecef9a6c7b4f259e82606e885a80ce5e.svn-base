...
import com.wgb.service.dubbo.stms.admin.ApiLoadingDataService;
...
public class MTLoadingDataFomSTMSController extends MTBaseController {

    @Autowired
    private ApiLoadingDataService apiLoadingDataServiceFromSTMS;

    @RequestMapping("/getLoadingSuppliersData")
    @ResponseBody
    public ZLResult getLoadingSuppliersIdsData(HttpServletRequest request) {
        // 获取入参数据
        ...
        try {
            // 系统调用
            zlRpcResult = apiLoadingDataServiceFromSTMS.getLoadingSuppliersData(params);
        } catch (Exception ex){
            // 系统级别异常
