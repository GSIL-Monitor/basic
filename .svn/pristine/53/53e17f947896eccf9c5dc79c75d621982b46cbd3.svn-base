...
public class RenewController extends SPBaseController {
    ...
    @ResponseBody
    public ZLResult writeRenew(HttpServletRequest request) {
        ZLResult result = ZLResult.Success();
        try {
            Map<String, Object> params = getParams();
            ZLRpcResult zlRpcResult= apitRenewService.writeRenew(params);
            result = parseRpcResult(zlRpcResult);
        }catch (Exception ex){
            LOGGER.error("跳转链接失败!",ex);
            result.setErrorMsg("操作失败，请联系管理员！");
        }
        return result;
    }
    ...
    @ResponseBody
    public ZLResult queryaleassistantcode(HttpServletRequest request) {
        ZLResult result = ZLResult.Success();
        try {
            Map<String, Object> params = getParams();
